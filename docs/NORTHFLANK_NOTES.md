...
2. **Sandbox compute is always-on** — the pricing page states
   _"Always-on-compute – no sleeping :)"_. This is the reason we moved off
...
billing page says it plainly: _"all users must add a payment method to start
creating resources on Northflank, regardless of plan selection"_, for identity
...
- **Secrets belong in `argumentOverrides`, never in `arguments` or the spec.**
  The docs are explicit: _"All sensitive secrets should be stored as argument
  overrides, not within the template or as template arguments."_ Overrides are
...
1. **The top level allows only `apiVersion`, `arguments` and `spec`**
   ...
   file — are rejected there. They belong to the API request that _runs_ the
...
One caveat, stated plainly: a `${args.x}` string satisfies _both_ branches of
...
`npm i -g @northflank/cli`, then `northflank login` — _"a browser window will
open where you can select an API token"_ — or `northflank login -t <token>`
